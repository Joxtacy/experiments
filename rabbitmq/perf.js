...
function plot_series(div, dimensions, dimension_values, data) {
	...
	$.each(series_values, function (i, s_val) {
		...
		$.each(x_values, function (j, x_val) {
			var val = series_first ? data[s_val][x_val] : data[x_val][s_val];
...
function plot_data(div, chart_data, extra) {
	...
	row.after(
		'<tr><td></td><td class="xaxis">' + div.attr("data-x-axis") +
		"</td><td></td></tr>",
	);
...
var axes_rate_and_bytes = [{ min: 0 }, { min: 0, position: "right" }];

var log_x_axis = { min: 1, transform: log_transform, ticks: log_ticks };
