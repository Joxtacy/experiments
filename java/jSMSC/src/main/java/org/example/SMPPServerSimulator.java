...
public class SMPPServerSimulator extends ServerResponseDeliveryAdapter implements Runnable, ServerMessageReceiverListener {
	...
	public SubmitSmResult onAcceptSubmitSm(SubmitSm submitSm,
		...
		log.info("Receiving submit_sm '{}', and return message id {}, from {}", decodeShortMessage(submitSm.getShortMessage(), dataCoding), messageId, submitSm.getSourceAddr());
	...
	private static String decodeShortMessage(byte[] shortMessage, byte dataCoding) {
		Charset charset = switch (dataCoding) {
			//case 0x00 -> // GSM 7-bit default alphabet
			//Charset.forName("GSM-7"); // Note: You'll need to implement or find a GSM 7-bit decoder
			case 0x08 -> // UCS2 (ISO/IEC-10646)
							StandardCharsets.UTF_16BE;
			case 0x04 -> // 8-bit data
							StandardCharsets.ISO_8859_1;
			default -> StandardCharsets.UTF_8; // Default to UTF-8 for unknown encodings
		};
		return new String(shortMessage, charset);
	}
