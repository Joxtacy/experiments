...
import wisp_app/models/item.{type Item, Completed, Uncompleted}

pub fn root(items: List(Item)) -> Element(t) {
  html.div([attribute.class("app")], [
    html.h1([attribute.class("app-title")], [element.text("Todo App")]),
    todos(items),
  ])
}
...
fn item(item: Item) -> Element(t) {
  ...
  html.div([attribute.class("todo " <> completed_class)], [
    html.div([attribute.class("todo__inner")], [
      ...
      html.span([attribute.class("todo__title")], [element.text(item.title)]),
    ]),
    html.form(
      [
        attribute.method("POST"),
